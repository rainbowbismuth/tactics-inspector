...
//        fontAtlas.addFontFromFileTTF("src/main/resources/Righteous-Regular.ttf", 14, fontConfig);
//        fontAtlas.addFontFromFileTTF("src/main/resources/Righteous-Regular.ttf", 16, fontConfig);

        // Or directly from the memory
        fontConfig.setName("VT323-Regular.ttf, 24px"); // This name will be displayed in Style Editor
        fontAtlas.addFontFromMemoryTTF(loadFromResources("VT323-Regular.ttf"), 24, fontConfig);
//        fontConfig.setName("VT323-Regular.ttf, 36px"); // We can apply a new config value every time we add a new font
//        fontAtlas.addFontFromMemoryTTF(loadFromResources("VT323-Regular.ttf"), 36, fontConfig);
    ...
    private void loop() throws Exception {
        ...
        while (!glfwWindowShouldClose(windowPtr)) {
            ...
            ImGui.newFrame();
            inspectorUI.render(winWidth[0], winHeight[0]);
            ImGui.render();
    ...
    private void destroyGlfw() {
        for (final long mouseCursor : mouseCursors) {
    ...
    private byte[] loadFromResources(final String fileName) {
        try (final InputStream is = Objects.requireNonNull(rainbowbismuth.fft.gui.Main.class.getClassLoader().getResourceAsStream(fileName));
